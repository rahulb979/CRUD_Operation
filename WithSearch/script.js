...
function displayRecords(arrayToCheck, pageNumber, pageSize) {
    displayRecords(arrayToCheck, pageNumber, pageSize, false);
}


function displayRecords(arrayToCheck, pageNumber, pageSize, isChangeDisabled) {
    removeCurrentDisplay();
    paginatedArr = paginate(arrayToCheck, pageSize, pageNumber);
    for (var i = 0; i < paginatedArr.length; i++) {
        formObj = getFormObject(paginatedArr[i]);
        insertNewRecord((pageNumber-1)*pageSize+(i+1),formObj, false, isChangeDisabled);
    }
...
function init() {
    updateSummary();
    displayRecords(recordArray, curPage, pageSize);
    document.getElementById('clearSearch').setAttribute("disabled", true);
}
...
// Create operation
function insertNewRecord(rowid, data, fl, isChangeDisabled){
...
function previousButtonClicked(){
    ...
    displayRecords(recordArray, curPage-1, pageSize);
...
function toggleButtonsOFF() {
    document.getElementById('recordCount').setAttribute("disabled", true);
    ...
    document.getElementById('clearSearch').removeAttribute("disabled");
    document.getElementById('curPage').innerText = 1;    
}

function toggleButtonsON() {
    document.getElementById('recordCount').removeAttribute("disabled");
...
function searchButtonClicked(searchObj) {
    clearOtherSearches(searchObj.id);
    ...
    switch (searchObj.id) {
        case "search1":
            for (var i = 0; i < recordArray.length; i++) {
                ...
                if (record[0].includes(searchVal) ){
                    searchedArray.push(record);
                }
            }
        ...
        case "search2":
            for (var i = 0; i < recordArray.length; i++) {
                var record = recordArray[i];
                if (record[1].includes(searchVal) ){
                    searchedArray.push(record);
                }
            }
        ...
        case "search3":
            for (var i = 0; i < recordArray.length; i++) {
                var record = recordArray[i];
                if (record[2] == parseInt(searchVal) ){
                    searchedArray.push(record);
                }
            }
        ...
        case "search5":
            for (var i = 0; i < recordArray.length; i++) {
                var record = recordArray[i];
                if (record[4].includes(searchVal) ){
                    searchedArray.push(record);
                }
            }
        ...
        default:
            for (var i = 0; i < recordArray.length; i++) {
                var record = recordArray[i];
                if ((record[0] + " " + record[1]).includes(searchVal) ){
                    searchedArray.push(record);
                }
            }
    ...
    displayRecords(searchedArray, 1, searchedArray.length, true);

    toggleButtonsOFF();
}

function clearOtherSearches(searchId) {
    for (var i = 1 ; i <=5 ;i++) {
        var tempId = 'search' + i;
        if (tempId == searchId) {
            continue;
